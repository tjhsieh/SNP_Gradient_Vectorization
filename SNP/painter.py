...
class PainterBase():
    ...
    def _render(self, v, save_jpgs=True, save_video=True):
        ...
        for i in range(v.shape[0]):  # for each stroke
            ...
            this_single_frame = self.rderr.single_canvas
            this_single_frame = cv2.resize(this_single_frame, (out_w, out_h), cv2.INTER_AREA)
            if save_jpgs:
                plt.imsave(file_name + '_rendered_stroke_' + str((i+1)).zfill(4) +
                           '.png', this_frame)
                plt.imsave(file_name + '_rendered_single_stroke_' + str((i+1)).zfill(4) +
                           '.png', this_single_frame)
            if save_video:
