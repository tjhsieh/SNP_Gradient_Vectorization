...
class sparse_coord_init():
    def __init__(self, pred, gt, format='[bs x c x 2D]', quantile_interval=200, nodiff_thres=0.1):
        ...
        #quantized_interval = sorted(quantized_interval)
        quantized_interval = sorted(quantized_interval[1:])
...
def init_shapes(num_paths,
    ...
    for i in range(num_paths):
        ...
        if 'gt' in kwargs:
            ...
            #fill_color_init = torch.FloatTensor(np.append(npr.uniform(size=[3]),1))
            stroke_color_init = torch.FloatTensor(npr.uniform(size=[4]))
...
if __name__ == "__main__":
    ...
    name = cfg.target.split('/')
    name = name[-1].split('.')
    cfg.experiment_dir = \
        osp.join(cfg.log_dir,name[0] ) #'{}_{}'.format(cfg.exid, '_'.join(cfg.signature))
    ...
    for path_idx, pathn in enumerate(path_schedule):
        ...
        pathn_record_str = str(path_idx+1) #'-'.join([str(i) for i in pathn_record])
        ...
        for t in t_range:
            ...
            # save video-svg
                filename = os.path.join(
                    cfg.experiment_dir, "video-svg", "{}-iter{}.svg".format(pathn_record_str, t))
                check_and_create_dir(filename)
    ...
    #direct = "output/path1" + name[0] + '.svg'
    #current = "log/" + name[0] + "/output-svg/1.svg"
    #shutil.move( current, direct )
